...
<body>
    ...
    @foreach ($data as $row)
    <div class="container">
        <form action="/berita-open" method="GET">
            {{ csrf_field() }}
            <input type="hidden" name="id" value="{{ $row->id }}">
        <div class="row">
            <p class="text-center fs-3 fw-bold mt-4 mb-4" style="font-weight:600; color:black; ">
                 {{ $row->isi_berita }} <strong class="fw-bold" style="font-weight:600; color:red; "></strong></p>
        </form>
        </div>
        @endforeach
    </div>
